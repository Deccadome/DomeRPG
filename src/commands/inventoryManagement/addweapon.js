...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("addweapon")
    .setDescription(
      "Add a weapon to your primary character (must have a primary character to use)."
    )
  ...
  async autocomplete(interaction, client) {
    ...
    for (
      let weapon = await cursor.next();
      weapon != null;
      weapon = await cursor.next()
    ) {
  ...
  async execute(interaction, client) {
    curCharacter = await Character.findOne({
    ...
    if (!curCharacter) {
      await interaction.reply({
        content:
          "You do not have a current primary character. Create a new one with /newcharacter or switch to an existing one with /changecharacter.",
        ephemeral: true,
      });
    } else {
